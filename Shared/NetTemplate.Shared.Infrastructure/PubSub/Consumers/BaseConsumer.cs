...
namespace NetTemplate.Shared.Infrastructure.PubSub.Consumers
{
    public abstract class BaseConsumer<TConsumer, TKey, TModel>
    {
        ...
        public Task Start(CompetingConsumerConfig commonConfig, CancellationToken cancellationToken = default)
        {
            ...
            for (var i = 1; i <= consumerConfig.ConsumerCount; i++)
            {
        ...
        {
            using IConsumer<TKey, TModel> consumer = KafkaHelper.CreateConsumer<TKey, TModel>(consumerConfig);
            ...
            try
            {
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ConsumeResult<TKey, TModel> message = consumer.Consume(cancellationToken);

                    using (IServiceScope scope = provider.CreateScope())
                    {
                        await Handle(message.Message.Value, scope.ServiceProvider, cancellationToken);
                    }
        ...
        protected abstract Task Handle(TModel model, IServiceProvider provider, CancellationToken cancellationToken = default);
    }
}
